...
# First part of the file (up to line 716)
for ($i = 0; $i -lt 716; $i++) {
    $newContent += $content[$i]
}

# The new enrollment case
$newEnrollment = @"
            case 'enrollment':
...
$newContent += $newEnrollment -split "`r`n"

# Last part of the file (from line 870 onwards)
for ($i = 870; $i -lt $content.Count; $i++) {
    $newContent += $content[$i]
}
